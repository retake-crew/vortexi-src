...
fn get_installation_directory() -> PathBuf {
    return PathBuf::from(data_local_dir().unwrap().to_str().unwrap()).join("Vortexi");
}
...
async fn main() {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        bootstrapper_filename = "VortexiPlayerLinuxLauncher";
    }
    ...
    if terminal_width < 80 {
        print!("{}\n", format!("Vortexi Bootstrapper | {} | Build Date: {} | Version: {}", base_url, build_date, env!("CARGO_PKG_VERSION")).to_string().magenta().cyan().italic().on_black()); // Fallback message
    } else {
        ...
        for line in &startup_text_lines[0..startup_text_lines.len() - 1] {
            ...
            println!("{}", formatted_line.bright_green().bold().on_black());  
        }
        ...
        println!("{}\n", last_line.yellow().bold().on_black());
    }
...
MimeType=x-scheme-handler/cayneez-player;", latest_bootstrapper_path.to_str().unwrap(), latest_bootstrapper_path.to_str().unwrap(), env!("CARGO_PKG_VERSION"));
    ...
    if !app_settings_path.exists() { //|| !client_executable_path.exists() {
        ...
        let Client2018Zip : PathBuf = download_file_prefix(&http_client, format!("{}client.zip", VersionURLPrefix).as_str(), &temp_downloads_directory).await;
...
<Settings>
    ...
    client_executable_path = current_version_directory.join("Client2018").join("VortexiPlayerBeta.exe");
    ...
    if !client_executable_path.exists() {
        ...
        error("Failed to run VortexiPlayerBeta.exe, is your antivirus removing it? The bootstrapper will attempt to redownload the client on next launch.");
        std::thread::sleep(std::time::Duration::from_secs(20));
        std::process::exit(0);
    }
    match launch_mode.as_str() {
        "play" => {
            info("Launching Vortexi");
            #[cfg(target_os = "windows")]
            {           
                let mut command = std::process::Command::new(client_executable_path);
                
                if client_year == "2015" {
                    // Use the 2015 format with -a, -j, -t arguments
                    command.args(&[
                        "-a", format!("http://{}/Login/Negotiate.ashx", base_url).as_str(),
                        "-j", join_script.as_str(),
                        "-t", authentication_ticket.as_str()
                    ]);
                } else {
                    // Use the standard format for all other clients
                    command.args(&[
                        "--play",
                        "--authenticationUrl", format!("https://{}/Login/Negotiate.ashx", base_url).as_str(),
                        "--authenticationTicket", authentication_ticket.as_str(),
                        "--joinScriptUrl", format!("{}", join_script.as_str()).as_str()
                    ]);
                }
            ...
            #[cfg(not(target_os = "windows"))]
            {
                ...
                if client_year == "2015" {
                    // Use the 2015 format with -a, -j, -t arguments for wine too
                    command.args(&[
